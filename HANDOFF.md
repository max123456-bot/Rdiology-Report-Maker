...
| `corpus.py` | The clinic library: offline term extraction from uploaded books/papers (PDF/DOCX/TXT), a shared per-tenant corpus (capped 50k terms) stored under the reserved `__clinic_corpus__` name, a sound-key-bucketed TermIndex for near-miss suggestions at book scale, and top-K relevance selection feeding STT biasing + AI prompts. Also keeps reference CHUNKS of each uploaded document, retrievable with citations ("Ask the clinic library" in the Draft tab). Suggest-only, like everything else. |
| `memory.py` | Agentic memory: semantic (uncapped style-rule archive with embeddings) + episodic (accepted reports) per doctor, retrieval scored cosine x recency-decay (180-day half-life) x confidence, Gemini embeddings with a deterministic offline hash fallback, and the preference-pairs DPO archive (every notes/draft/final-edit triple, JSONL-exportable). Reserved names `__agent_memory__` / `__preference_pairs__`; Template.preferences stays the always-sent hot list. |
